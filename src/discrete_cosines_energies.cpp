...
namespace pndl {
  ...
  DiscreteCosinesEnergies::DiscreteCosinesEnergies(const ACE& ace): AngleEnergy(std::make_shared<Region1D>(std::vector<double>({0.,200.}), std::vector<double>({1.,1.}), Interpolation::LinLin)), incoming_energy_(), outgoing_energies_(), Noe(0), Nmu(0), skewed_(false) {
    // Make sure the distributions is discrete cosines and energies
    int32_t nxs_7 = ace.nxs(6);

    if (nxs_7 != 0 && nxs_7 != 1) {
      std::string mssg = "DiscreteCosinesEnergies::DiscreteCosinesEnergies: The provided ACE file does not contain a distribution of this form for Incoherent Inelastic scattering.";
    ...
    if (!std::is_sorted(incoming_energy_.begin(), incoming_energy_.end())) {
      std::string mssg = "DiscreteCosinesEnergies::DiscreteCosinesEnergies: The incident energy grid is not sorted.";
      throw PNDLException(mssg, __FILE__, __LINE__);
    }
    ...
    if (skewed_ && Noe < 6) {
      std::string mssg = "DiscreteCosinesEnergies::DiscreteCosinesEnergies: A skewed distribution must have at least 6 outgoing energies. Only " + std::to_string(Noe) + " were provided.";
      throw PNDLException(mssg, __FILE__, __LINE__);
    }
    ...
    for(size_t ie = 0; ie < Ne; ie++) {
      ...
      for(size_t oe = 0; oe < Noe; oe++) {
        ...
        // Check E_out
        if (E_out <= 0.) {
          std::string mssg = "DiscreteCosinesEnergies::DiscreteCosinesEnergies: Nevative outgoing energy found at index " + std::to_string(i) + ".";
          throw PNDLException(mssg, __FILE__, __LINE__);
        }
        i++;
        ...
        for(size_t j = 0; j < Nmu; j++) {
          if (mu[j] < -1. || mu[j] > 1.) {
            std::string mssg = "DiscreteCosinesEnergies::DiscreteCosinesEnergies: Invalid cosine value found at index " + std::to_string(i+j) + ".";
            throw PNDLException(mssg, __FILE__, __LINE__);
          }
        }
        i += Nmu;
  ...
  AngleEnergyPacket DiscreteCosinesEnergies::sample_angle_energy(double E_in, std::function<double()> rng) const {
    uint32_t j = 0;
    uint32_t k = Nmu * rng();
    ...
    auto Eit = std::lower_bound(incoming_energy_.begin(), incoming_energy_.end(), E_in);
    if (Eit == incoming_energy_.begin()) {
      // Below lowest incident energy.
      return {outgoing_energies_.front()[j].cosines[k], outgoing_energies_.front()[j].energy};
    } else if (Eit == incoming_energy_.end()) {
      // Above largest incident energy.
      return {outgoing_energies_.back()[j].cosines[k], outgoing_energies_.back()[j].energy};
    }
    uint32_t i = std::distance(incoming_energy_.begin(), Eit) - 1;
    double f = (E_in - incoming_energy_[i])/(incoming_energy_[i+1] - incoming_energy_[i]);
