...
struct Sigma1 {
  ...
  inline static double broaden(std::span<const double> egrid,
                               std::span<const double> xs, double E,
...
#ifdef _OPENMP
#pragma omp simd reduction(+ : sig) private(Fa, Fb, H)
#endif
    for (std::size_t k = low; k < hi; k++) {
      double x_k = xgrid[k];
      double x_k1 = xgrid[k + 1];
      double sk = (xs[k + 1] - xs[k]) / (x_k1 * x_k1 - x_k * x_k);
      fill_f(Fa, x_k - y);
      fill_f(Fb, x_k1 - y);
      fill_h(Fa, Fb, H);
