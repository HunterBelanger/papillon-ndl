...
Nuclide& get_nuclide(const hid_t h5file) {
  ...
  openmc::data::nuclides.push_back(nullptr);
  openmc::data::nuclides[0] =
      std::make_unique<Nuclide>(grp_id, std::vector<double>(1, 293.6));
  openmc::close_group(grp_id);

  return *openmc::data::nuclides[0];
}

std::unique_ptr<AngleEnergy> get_coherent_elastic(const hid_t h5file,
                                                  const std::string& tmpgroup) {
...
std::unique_ptr<AngleEnergy> get_incoherent_elastic(
    const hid_t h5file, const std::string& tmpgroup) {
  std::unique_ptr<AngleEnergy> ae = nullptr;
  ...
  if (openmc::object_exists(temp_grp_id, "elastic")) {
    ...
    if (temp == "incoherent_elastic") {
      ...
      ae = std::make_unique<openmc::IncoherentElasticAEDiscrete>(dgroup,
                                                                 xs->x());
    } else {
...
std::unique_ptr<AngleEnergy> get_incoherent_inelastic(
    const hid_t h5file, const std::string& tmpgroup) {
  std::unique_ptr<AngleEnergy> ae = nullptr;
  ...
  if (openmc::object_exists(temp_grp_id, "inelastic")) {
    ...
    if (temp == "incoherent_inelastic") {
      ...
      ae = std::make_unique<openmc::IncoherentInelasticAEDiscrete>(dgroup,
                                                                   xs->x());
    } else {
...
int reaction(const std::uint32_t mt, const std::uint64_t nsamples,
  ...
  if (Ein <= reaction->xs_[0].threshold) {
    std::cerr << " Incident energy " << Ein << " eV is less than threshold of "
              << reaction->xs_[0].threshold << ".";
    return 1;
  }
  ...
  for (std::uint64_t n = 0; n < nsamples; n++) {
    ...
    if (CM) {
      ...
      Eout = E_cm + (Ein + 2.0 * mu * (AWR + 1.0) * std::sqrt(Ein * E_cm)) /
                        ((AWR + 1.0) * (AWR + 1.0));
      mu = mu * std::sqrt(E_cm / Eout) +
           1.0 / (AWR + 1.0) * std::sqrt(Ein / Eout);
    }
...
int elastic(ElasticMode mode, const std::uint64_t nsamples, const double Ein,
            const double T, const hid_t h5file, NDArray<double>& data) {
  openmc::settings::res_scat_on = true;

  // Get the nuclide
  auto& nuclide = get_nuclide(h5file);

  openmc::settings::res_scat_energy_max = 1.E200;
  openmc::settings::res_scat_energy_min = 0.;

  // Temperature in eV
  const double kT = (8.617333262E-5) * T;

  // Get elastic reaction from nuclide
  const auto& rx = *nuclide.reactions_[0];

  if (mode == ElasticMode::SVT && nuclide.resonant_ == true) {
    nuclide.resonant_ = false;
  } else if (mode == ElasticMode::DBRC && nuclide.resonant_ == false) {
    std::cerr << " Cannot use DBRC as no 0K elastic xs is provided.";
    return 1;
  }

  if (mode == ElasticMode::SVT) {
    openmc::settings::res_scat_method = openmc::ResScatMethod::cxs;
  } else {
    openmc::settings::res_scat_method = openmc::ResScatMethod::dbrc;
  }

  // Make the particle which will be used as a template
  openmc::model::n_coord_levels = 1;
  openmc::Particle p;
  p.E() = Ein;
  p.r() = {0., 0., 0.};
  const openmc::Direction u_orig{0., 0., 1.};
  p.u() = u_orig;
  p.stream() = 0;
  p.seeds(0) = seed;
  ...
  // Get all samples
  for (std::uint64_t n = 0; n < nsamples; n++) {
    openmc::elastic_scatter(0, rx, kT, p);
    double Eout = p.E();
    double mu = u_orig.dot(p.u());
...
const std::string HELP_STR =
    ...
    "  sopenmc elastic-svt <h5file> <nsamples> <energy> <T> <npyfile>\n"
    "  sopenmc elastic-dbrc <h5file> <nsamples> <energy> <T> <npyfile>\n"
    "  sopenmc coherent-elastic <h5file> [--temp <tmpgroup>] <nsamples> "
    "<energy> <npyfile>\n"
    "  sopenmc incoherent-elastic <h5file> [--temp <tmpgroup>] <nsamples> "
    "<energy> <npyfile>\n"
    "  sopenmc incoherent-inelastic <h5file> [--temp <tmpgroup>] <nsamples> "
    "<energy> <npyfile>\n\n"
...
int main(int argc, char** argv) {
  ...
  if (mode != RunMode::REACTION && mode != RunMode::ELASTIC_SVT &&
      mode != RunMode::ELASTIC_DBRC && E > 4.) {
  ...
  if (result == 0) {
    switch (mode) {
      case RunMode::REACTION:
        ...
        break;
      case RunMode::ELASTIC_SVT:
        result = elastic(ElasticMode::SVT, NSAMPLES, E, T, h5_file_id, data);
        break;
      case RunMode::ELASTIC_DBRC:
        result = elastic(ElasticMode::DBRC, NSAMPLES, E, T, h5_file_id, data);
        break;
      case RunMode::COHERENT_ELASTIC:
      ...
      case RunMode::INCOHERENT_INELASTIC:
        result =
            incoherent_inelastic(NSAMPLES, E, h5_file_id, temp_group, data);
        break;
    }
  }
