...
def leapr_interpolator(base_xml_model, new_leapr_input, temps):
    try:
    ...
    for temp in temps:
        if (temp != None):
            ...
            new_temps.append(new_temperature(model, temp))
            ntempr += 1

    if (len(new_temps) > 0):
        model.leapr.ntempr = "%i" % (ntempr)
        model.leapr.title = "Model interpolated at %i temperatures between %.2f and %.2f" % (ntempr, min_temp, max_temp)
        model.leapr.temperature = new_temps
...
if __name__ == '__main__':
    ...
    # Number of provided temperatures.
    ntemps = len(sys.argv) - 3
    temps = []
    for i in range(ntemps):
        try:
            t = float(sys.argv[3+i])
        except ValueError:
            print("Error: %s does not seem to be a float" %(sys.argv[3+i]))
            raise SystemExit
        temps.append(t)
    # Ensure temperatures are sorted !
    temps.sort()
    leapr_interpolator(base_xml_model, new_leapr_input, temps)
